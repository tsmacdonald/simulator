...
public class AgentStateCondition extends EndCondition {
	 ...
	 * @param fieldName The name of the field to check within the 'target' type of agent
	 * @param fieldVal The specified value of fieldName that triggers the ending condition
	 */
	public AgentStateCondition(Grid grid, String agentName, int populationLimit, String fieldName, String targetVal) {
		super(grid);
	...
	public boolean evaluate() {
		...
		//TODO: Get the Agent team to get the Field class figured out and provide a means for accessing a Field's value
		while(it.hasNext()){	
			Slot s = it.next(); 
			
			if(s.getEntity().name().equals(agentName)){
				if(s.getEntity().getField(fieldName).getFieldValue.equals(targetVal)){
					count++;
				}
